...
class RtcxClient:
    ...
    async def _setting(self, iot_id: str, cmd: int, payload: bytes) -> None:
        # Held across the settle as well as the write: releasing the moment
        # the request returns would let the next frame land in the slot before
        # the charger has taken this one out of it. That the setting would then
        # be lost with everything reporting success is what the slot allows,
        # not something anyone has seen -- see SETTING_SETTLE_SECONDS. The
        # sleep's place inside this block is what makes the precaution mean
        # anything, so a test pins it there.
        async with self._talk:
            await self.call(
                "/client/thing/properties/set",
    ...
    async def async_set_charging_mode(
        ...
        the reported bytes exactly. The hazard is an edit: the app can change
        a group's protocol mask along with its limit -- the shared C6+A slider
        ...
        Replaying is measured. Sending a `GET_DEVICE_STATE` reply's 36 bytes
        ...
        on. The charger also takes a one-field edit: C4 moved from 60 W to 55 W
        landed alone. That it accepts one is not the question; whether the app
        would ever send the pair it leaves is. And the round trip this exists
        for: a priority port set to C2 in the app survived
