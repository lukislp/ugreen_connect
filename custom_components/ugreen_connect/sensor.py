...
class UgreenCustomLimitSensor(UgreenDeviceEntity, SensorEntity):
    ...
    This is the mode the app calls its own editor. Created the first time the
    charger is seen in `custom`, unavailable while any other mode runs, and
    absent altogether on a charger never seen in it -- under a preset those
    same bytes are that preset's own settings, so there is no custom
    configuration to report. Diagnostic rather than beside the live readings,
    because they describe how the charger is set up rather than what it is
    doing.
